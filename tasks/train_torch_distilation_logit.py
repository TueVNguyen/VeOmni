...
def main():
    ...
    init_parallel_state(
        ...
        hsdp_size=args.train.hybrid_fsdp_size,
    )
    ...
    if args.data.dataloader_type == "native":
        logger.info_rank0("Start building simple dataloader")
        train_dataset = MultiTurnSFTDataset(
            data_files=args.data.train_path,
            tokenizer=tokenizer,
            messages_key=args.data.text_keys,
            max_length=args.data.max_seq_len,
            cache_path=os.environ.get("HF_CACHE_DIR", None),
            is_chatml=args.data.is_chatml
        )
        args.data.train_size = train_dataset.total_tokens
        args.data.set_train_size(args.data.train_size)
        args.train.compute_train_steps(args.data.max_seq_len, args.data.train_size, len(train_dataset), legacy_compute=True)

        parallel_state = get_parallel_state()
        sampler = DistributedBatchMultiTurnSFTDatasetSampler(
            ...
            corss_pack=args.data.corss_pack
        )
        train_dataloader = build_dataloader_v2(
            dataset=train_dataset,
            sampler=sampler,
            micro_batch_size=args.train.micro_batch_size,
            global_batch_size=args.train.global_batch_size,
            dataloader_batch_size=args.train.dataloader_batch_size * 2,
            seed=args.train.seed,
            max_seq_len=args.data.max_seq_len,
            ...
            token_micro_bsz=args.train.token_micro_bsz,
    ...
    if args.train.load_checkpoint_path:
        ...
        dist.barrier()
        logger.info(f"Load distributed checkpoint from {args.train.load_checkpoint_path} successfully!")
    ...
    for epoch in range(start_epoch, args.train.num_train_epochs):
        ...
        for _ in range(start_step, args.train.train_steps):
            ...
            n_item = sum([
                torch.where(micro_batch["labels"] != -100, 1., 0.).sum()
            ...
            total_loss, grad_norm = all_reduce((total_loss, grad_norm), "mean", group=None) 
            torch.cuda.synchronize()
            ...
            if args.train.save_steps and global_step % args.train.save_steps == 0 or global_step == 5: 
                if global_step == 5:
                    logger.info_rank0(f"Testing save checkpoint")
                helper.empty_cache()

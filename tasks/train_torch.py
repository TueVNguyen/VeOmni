...
def main():
    ...
    for epoch in range(start_epoch, args.train.num_train_epochs):
        ...
        for _ in range(start_step, args.train.train_steps):
            ...
            n_item = all_reduce(n_item, "sum", group=get_parallel_state().fsdp_group)
            ddp_size = torch.distributed.get_world_size(get_parallel_state().fsdp_group)
            n_item = n_item / ddp_size # please check the formular 
            # get group_by sequence: 
            #    loss_group= sum(l_i) 
            #    total_loss_real = [loss_group1 + loss_group2 + ... + loss_group_n ]//  *world_size
            #                            = sum (loss)  // [world_size x seq_size ] = sum(loss) x world_size 
            #
            #    g = g1 / world_size + g1/world_size = 2xg1/world_size ..
            #    G = seq_size x [g1+g2+g3] / world_size = [g1+g2+g3] / [world_size / seq_size] 
            for micro_batch in micro_batches:
                ...
                with model_fwd_context:
                    ...
                    if get_parallel_state().sp_enabled:
                        from veomni.distributed.sequence_parallel.loss import ReduceLoss2
                        loss = ReduceLoss2.apply(loss, get_parallel_state().sp_group)
            ...
            total_loss, grad_norm = all_reduce((total_loss, grad_norm), "mean", group=get_parallel_state().fsdp_group) 

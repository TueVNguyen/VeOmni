...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.training and get_parallel_state().ep_enabled:
            ...
            num_local_experts = self.num_experts // get_parallel_state().ep_size
...
class Qwen3MoeForCausalLM(Qwen3MoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        print(f"Qwen3MoeForCausalLM init")
...
if is_liger_kernel_available():
    ...
    class LigerSwiGLUMLPMoe(nn.Module):
        def __init__(self, config, intermediate_size=None):
            super().__init__()
            self.config = config
            self.hidden_size = config.hidden_size
    ...
    Qwen3MoeMLP = LigerSwiGLUMLPMoe
    logger.info_rank0("Apply liger kernel to Qwen3MOE.")

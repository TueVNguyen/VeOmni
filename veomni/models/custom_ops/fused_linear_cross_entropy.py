...
class FusedLinearCrossEntropyRecompute(torch.autograd.Function):
    """Memory-efficient version of FusedLinearCrossEntropy that recomputes grad_weight during backward."""
    ...
    def forward(
        ...
        # Run forward pass but don't store grad_weight
        loss, z_loss, grad_input, _, grad_bias = fused_linear_cross_entropy_forward(
            _input=_input,
            weight=weight,
            target=target,
            bias=bias,
        ...
        # Save tensors needed for backward and recomputation
        ctx.save_for_backward(
            _input,
            weight,
            target,
    ...
    def backward(ctx, grad_output, grad_output2):
        ...
        # Recompute grad_weight if weight requires gradient
        grad_weight = None
        if weight.requires_grad:
            ...
            # Recompute only the grad_weight part
            target_mask = target != ignore_index
            total_n_non_ignore = target_mask.sum().item()
            total_sum_non_ignore_ce_weight = total_n_non_ignore
            ce_weight_sum = 0.0
            ...
            for chunk_id in range(num_chunks):
                ...
                n_rows = logits_chunk.shape[0]
                loss_1d_slice = torch.zeros(n_rows, dtype=torch.float32, device=device)
                z_loss_1d_slice = torch.zeros(n_rows, dtype=_input.dtype, device=device) if return_z_loss else None
                
                # Run kernel to get gradients for logits
                liger_cross_entropy_kernel[(n_rows,)](
                    X_ptr=logits_chunk,
                    X_stride=logits_chunk.stride(-2),
                    Y_ptr=target_chunk,
                ...
                # Update grad_weight using the computed gradients
                torch.addmm(
                    input=grad_weight,
        ...
        # Apply grad_output to gradients
        grad_input, grad_weight, grad_bias = fused_linear_cross_entropy_backward(
            grad_output, grad_input, grad_weight, grad_bias
        )

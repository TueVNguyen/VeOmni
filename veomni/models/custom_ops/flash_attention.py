# max_length = position_ids.max().item() + 1from typing import Optional, Tuple
...
def _flash_attention_forward(
    ...
    **kwargs,
    ...
    attn_output = flash_attn_varlen_func(
        ...
        max_seqlen_q=max_length_q.item(),
        max_seqlen_k=max_length_k.item(),
        dropout_p=dropout,
        softmax_scale=softmax_scale,
        causal=causal,
        **flash_kwargs,
...
def flash_attention_forward(
    ...
    **kwargs,
    ...
    assert kwargs.get('cu_seq_lens_q', None) is not None
    assert kwargs.get('cu_seq_lens_k', None) is not None
    assert kwargs.get('max_length_q', None) is not None
    assert kwargs.get('max_length_k', None) is not None

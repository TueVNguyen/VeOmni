...
from .batching_strategy import TextBatchingStrategy, KeepInOrderStrategy
...
def build_dataloader(
    ...
    seed: int = 0,
    token_micro_bsz: int = None,
    sampler: Optional[Callable] = None,
) -> "DistributedDataloader":
    parallel_state = get_parallel_state()
    if token_micro_bsz is None:
    ...
    if not isinstance(dataset, IterableDataset) and sampler is None:
        sampler = StatefulDistributedSampler(
            dataset,
    ...
    if use_rmpad:
        dataloader = DynamicBatchSizeDataLoader(
            ...
            post_collate_fn=DataCollatorRemovePadding(),
...
def build_dataloader_v2(
    ...
    if use_rmpad:
        if sampler is not None:
            batching_strategy = KeepInOrderStrategy(token_micro_bsz=token_micro_bsz, micro_batch_size=num_micro_batch)
        else:
            batching_strategy = TextBatchingStrategy(
                token_micro_bsz=token_micro_bsz - dyn_bsz_margin * max_seq_len,
                buffer_size=dyn_bsz_buffer_size,

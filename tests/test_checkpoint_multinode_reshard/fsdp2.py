...
class Layer(nn.Module):
    def __init__(self, hidden_size):
        super().__init__()
        self.net1 = nn.Linear(hidden_size, hidden_size * 1)
        self.relu = nn.ReLU()
        self.net2 = nn.Linear(hidden_size * 1, hidden_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--iterations", "-i", type=int, default=2, help="the number of training iterations to run")
    ...
    print(f"Rank {rank} of {world_size} is initialized")
    # torch.cuda.set_device(rank)

    # Define and initalize FSDP model and optimizer
    model = Model(HIDDEN_SIZE, LAYER_NUM).to("cuda")
...
#  torchrun --master_addr=localhost --master_port=6000 --nproc_per_node=8 --nnodes=1 tests/test_checkpoint_multinode_reshard/fsdp2.py --mode normal
#  torchrun --master_addr=localhost --master_port=6000 --nproc_per_node=4 --nnodes=1 tests/test_checkpoint_multinode_reshard/fsdp2.py --mode resume